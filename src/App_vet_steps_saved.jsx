...
function WorkflowPanel({ kindKey, sessionId }) {
  ...
  const queue = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/v1/vet/${encodeURIComponent(sessionId)}/${meta.path}/queue`,
        {
          method: "POST",
  ...
  const cancel = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/v1/vet/${encodeURIComponent(sessionId)}/${meta.path}/cancel`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json", accept: "application/json" },
        }
...
function SessionStateExplorer({ sessionId }) {
  ...
  const fetchState = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/v1/vet/${encodeURIComponent(sessionId)}/state`,
        { headers: { accept: "application/json" } }
      );
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      ...
      Object.keys(KINDS).map(async (k) => {
        ...
        try {
          const resp = await fetch(
            `${API_BASE}/api/v1/vet/${encodeURIComponent(sessionId)}/outputs/${encodeURIComponent(
              kind
            )}`,
            { headers: { accept: "application/json" } }
          );
